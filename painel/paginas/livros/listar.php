...
<small>
	...
	<thead> 
	<tr> 
	<th class="esc">Código da Unidade Escolar</th>	
	<th class="">Vaga</th>	
	<th class="esc">Categoria da Vaga</th>
	<th class="esc">Edição do Processo</th>
	<th class="esc">Unidade Escolar</th>
...
<td class="esc {$classe_ativo}">{$estoque}</td>
<td class="esc"><img src="images/livros/{$foto}" width="25px"></td>

<td>
	<big><a href="#" onclick="editar('{$id}','{$codigo}','{$titulo}','{$subtitulo}','{$autor}','{$ano}','{$editora}','{$edicao}','{$categoria}','{$foto}','{$local}','{$obs}','{$estoque}','{$quantidade}')" 
...
<big><a href="#" onclick="mostrar('{$codigo}','{$titulo}','{$subtitulo}','{$autor}','{$ano}','{$nome_editora}','{$edicao}','{$nome_categoria}','{$foto}','{$nome_local}','{$status}','{$obs}','{$dataF}','{$estoque}','{$quantidade}')" 
title="Mostrar Dados"><i class="fa fa-info-circle text-primary"></i></a></big>


<big><a href="#" onclick="emprestimosAtivos('{$id}','{$titulo}')" title="Mostrar Empréstimos Ativos"><i class="fa fa-exclamation-triangle text-danger"></i></a></big>
...
<div align="right" style="margin-top: 10px" class="verde">Total de Cadastros: {$linhas}</div>
HTML;
...
<script type="text/javascript">
	function editar(id, codigo, titulo, subtitulo, autor, ano, editora, edicao, categoria, foto, local, obs, estoque, quantidade){
	...
	function mostrar(codigo, titulo, subtitulo, autor, ano, editora, edicao, categoria, foto, local, status, obs, data, estoque, quantidade){
	...
	function emprestimosAtivos(id, titulo){
		    	
    	$('#id_emprestimo').val(id);
    	$('#titulo_emprestimo').text(titulo);

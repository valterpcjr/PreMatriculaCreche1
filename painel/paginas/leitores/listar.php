...
if($busca == '%%'){
	echo "Pesquise pelo nome do aluno ou cpf!";
  exit();
}


$query = $pdo->query("SELECT * FROM leitores where nome LIKE '$busca' or cpf LIKE '$busca' order by nome asc");
...
<small>
	...
	<tr> 
	<th>Nome do Aluno</th>	
	<th class="ocultar">Telefone</th>	
	<th class="ocultar">cpf</th>	
	<th class="esc">Data do Cadastro</th>	
		
	<th><h4>SOLICITAR VAGA</h4></th>
	</tr> 
	</thead> 
	<tbody>	
HTML;
...
<td class="ocultar">{$telefone}</td>
<td class="ocultar">{$cpf}</td>
...
<big><a href="#" onclick="emprestimo('{$id}', '{$nome}')" title="SOLICITAR VAGA"><i class="fa fa-hand-o-right {$classe_ativo2}"></i></a></big>

<big><a href="#" <i class="fa fa-list {$classe_ativo3}"></i></a></big>
